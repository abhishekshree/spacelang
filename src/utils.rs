...
pub(crate) fn take_while_wrapper(
...
pub(crate) fn extract_digits(s: &str) -> Result<(&str, &str), String> {
    take_while_wrapper(|c| c.is_ascii_digit(), s, "expected digits".to_string())
}
...
pub(crate) fn extract_whitespace_wrapper(s: &str) -> Result<(&str, &str), String> {
    take_while_wrapper(|c| c == ' ', s, "expected a space".to_string())
}
...
mod tests {
    ...
    fn do_not_extract_spaces1_when_input_does_not_start_with_them() {
        assert_eq!(
            extract_whitespace_wrapper("blah"),
